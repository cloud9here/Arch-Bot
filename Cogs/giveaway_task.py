            ...
            if int(time.time()) > data["end_time"] and not data["ended"]:
               ...
               # ctx = data["ctx"]
                ...
                result_embed2 = discord.Embed(
                    title="<:giveaway:1054086461726658661> {} ".format(data["prize"]),
                    color=0x030404,
                    description=f"Winner: {msg}\nHosted By: <@{host}>"
                )
                result_embed2.set_footer(icon_url=self.bot.user.display_avatar.url, text="Ventura | Giveaway Ended" if len(winners) == 1 else "Ventura | Giveaway Ended")
                ...
                result_embed.set_footer(icon_url=self.bot.user.display_avatar.url, text=f"Ventura | Giveaway Ended")
                ...
                view = View()
                view.add_item(btn)
                await giveaway_message.edit(content=f"<:giveaway:1054086461726658661> **GIVEAWAY ENDED** <:giveaway:1054086461726658661>", embed=result_embed2, view=view)
