...
class test(BasicView):
    ...
    @discord.ui.button(label=f"0",emoji=f'<:icon_GiveawayIcon:1214584516849696788>', style=discord.ButtonStyle.gray,custom_id=f'give')
    async def dare(self, interaction: discord.Interaction, button):
        ...
        try:
            ...
            if len(str(interaction.channel.name)) <= 4:
                await interaction.response.send_message(f"<:crosss:1212440602659262505> This Giveaway Has Been Ended.", ephemeral=True)
...
class give(commands.Cog):
      ...
      if interaction.type == discord.InteractionType.component:
        ...
        if str(interaction.message.id) in data:
          ...
          try:
              ...
              if str(interaction.user.id) not in giveaway_users:
                number = int(button.label) if button.label else 0
                btn = Button(label=str(number+1),emoji=f'<:icon_GiveawayIcon:1214584516849696788>', style=discord.ButtonStyle.gray,custom_id=f'give')
                view = View()
                view.add_item(btn)
    ...
    async def gstart(self, ctx):
        ...
        init = await ctx.send(embed=discord.Embed(
            title=f"<:icon_GiveawayIcon:1214584516849696788> New Giveaway ! <:icon_GiveawayIcon:1214584516849696788>",
            description=f"<:icon_GiveawayIcon:1214584516849696788> Please answer the following questions to finalize the creation of the Giveaway",
            color=self.color)
        ...
        for question in questions:
            embed = discord.Embed(
                title=f"Giveaway <:icon_GiveawayIcon:1214584516849696788>",
            ...
            try:
                ...
                await ctx.send(embed=discord.Embed(
                    title=f"Error",
                    color=self.color,
                    description=f"<:crosss:1212440602659262505> | You took too long to answer this question, Please setup the giveaway again ):."
                ))
                return
            else:
        ...
        try:
            ...
            await ctx.send("<:crosss:1212440602659262505> | You didn't mention the channel correctly, do it like {}.".format(ctx.channel.mention))
            return

        try:
            winners = abs(int(answers[3]))
            if winners == 0:
                await ctx.send(f"<:crosss:1212440602659262505> | You did not enter an positive number.")
                return
        except ValueError:
            await ctx.send(f"<:crosss:1212440602659262505> | You did not enter an integer.")
            return
        prize = answers[0].title()
        channel = self.bot.get_channel(channel_id)
        converted_time = convert(answers[2])
        if converted_time == -1:
            await ctx.send(f"<:crosss:1212440602659262505> | You did not enter the correct unit of time (s|m|d|h)")
        elif converted_time == -2:
            await ctx.send(f"<:crosss:1212440602659262505> | Your time value should be an integer.")
            return
        await init.delete()
        await question_message.delete()
        giveaway_embed = discord.Embed(
            title="<:icon_GiveawayIcon:1214584516849696788> {} ".format(prize),
            color=self.color,
            description=f'Ends: <t:{now + converted_time[0]}:R> (<t:{now + converted_time[0]}:f>)\n'
                        f'Hosted by: {ctx.author.mention}\n\n'
                        f'[Invite Me](https://youtu.be/Aq5WXmQQooo?si=OXpww9lC6Vg5eBHO)\n'
        ...
        giveaway_message = await channel.send(content="<a:Gifttt:1131840234213101691> **Giveaway** <a:Gifttt:1131840234213101691>", embed=giveaway_embed, view=action_row)
    ...
    async def greroll(self, ctx, message: int):
      ...
      if str(message) not in data:
        ...
        with open(f"giveaway_users/{data[str(message)]['button_id']}.txt", "r") as file:
          ...
          link = data[str(message)]["link"]
          host = data[str(message)]["host"]
          result_embed = discord.Embed(color=0x0d0d13,
