...
class music(commands.Cog):
    ...
    async def play(self, ctx, *, query):
        ...
        if urls:
            ...
            if not vc.is_playing():
                ...
                embed7 = discord.Embed(description=f"Added **[{track.title}]({ray})** to the queue.",colour=self.color)
    ...
    async def PlayerStart(self, payload: wavelink.TrackEventPayload) -> None:
        ...
        embed = discord.Embed(description=f"Started Playing [{track.title}]({ray})・[{duration_str}]({ray})", colour=self.color)
    ...
    async def skip(self, ctx: commands.Context):
        ...
        if vc.queue:
            ...
            await vc.play(track)
            embed6 = discord.Embed(description=f"Started playing: [{track.title}]({ray})",colour=self.color)
            await ctx.reply(embed=embed6, mention_author=False)
    ...
    async def nowplaying(self, ctx):
        ...
        embed6.description=f"[{vc.current.title}]({ray})・[{duration_str}]({ray})"
        await ctx.reply(embed=embed6, mention_author=False)
    ...
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def end(self, ctx, index: int):
        vc: wavelink.Player = ctx.voice_client
        if ctx.voice_client is None:
            embed = discord.Embed(description="I am not in a voice channel.", colour=self.color)
            return await ctx.reply(embed=embed, mention_author=False)            
        ...
        embed7 = discord.Embed(description=f"Successfully removed `[{removed.title}]({ray})` from Queue.", color=self.color)    
        await ctx.reply(embed=embed7, mention_author=False)
